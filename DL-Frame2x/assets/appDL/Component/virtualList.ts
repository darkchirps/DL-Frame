...
export default class virtualList extends cc.Component {
    ...
    // --- 性能优化缓存 ---
    private _viewSize: cc.Size = cc.size(0, 0); // 视图尺寸缓存（避免频繁调用getContentSize）
    ...
    scrollToIndex(index: number, duration: number = 0.3) {
        if (!this._inited || index < 0 || index >= this._actualNumItems) {
            cc.warn(`VirtualList: 索引 ${index} 越界，列表总数为 ${this._actualNumItems}`);
            return;
        }
        ...
        if (this._sizeType) { // 垂直方向
            ...
            // 边界处理：防止滚动超出范围
            const maxOffsetY = Math.max(0, this.content.height - this._viewSize.height);
            ...
            // 边界处理：防止滚动超出范围
            const maxOffsetX = Math.max(0, this.content.width - this._viewSize.width);
    ...
    private _setItemPosition(node: cc.Node, index: number) {
        ...
        // 获取 Item 自身的锚点偏移
        let anchorOffsetX = node.anchorX * this._itemSize.width;
        let anchorOffsetY = (1 - node.anchorY) * this._itemSize.height;
