...
class TipSystem {
    ...
    private static _prefabPromise: Promise<cc.Prefab> | null = null;
    ...
    private static async _getTipNode(): Promise<cc.Node> {
        // 从对象池获取节点
    ...
    private static _setupTipNode(node: cc.Node, content: string, tipId: number) {
        ...
        node.scale = TIP_CONFIG.SCALE_FACTOR;
        // 在节点上存储内容，用于回收时清理
    ...
    private static _createTipAnimation(node: cc.Node, tipId: number, content: string) {
        return cc.tween(node)
            .delay(TIP_CONFIG.DISPLAY_DURATION)
            .by(TIP_CONFIG.FADE_DURATION, {
                position: new cc.Vec3(0, TIP_CONFIG.MOVE_DISTANCE, 0),
                scale: -TIP_CONFIG.SCALE_FACTOR
            }, { easing: "quintIn" })
            .call(() => this._recycleTipNode(node, content))
            .start();
    }

    private static _removeTip(node: cc.Node, tweenObj: any) {
        // 停止动画
        tweenObj.stop();
        // 立即回收节点
    ...
    private static _recycleTipNode(node: cc.Node, content: string, immediate = false) {
        // 从活动提示中移除
