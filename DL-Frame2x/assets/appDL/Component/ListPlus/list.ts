...
export default class list extends cc.Component {
    ...
    @property({
        ...
        visible() {
            let val: boolean = !this.lackCenter;
            if (!val)
    ...
    bindRenderEvent(uiScr: any, handler: string) {
        ...
        this.renderEvent.component = cc.js.getClassName(uiScr);
    ...
    set numItems(val: number) {
        ...
        // 仅保留虚拟列表逻辑（virtual list）
        t._actualNumItems = t._numItems = val;
        t._forceUpdate = true;

        // always treat as virtual list
        t._resizeContent();
        if (t.cyclic) {
    ...
    // 清空 content — 使用统一简洁实现，移除旧实现注释以便维护
    ...
    _onScrolling(ev: cc.Event = null) {
        ...
        // virtual-list only: compute displayData
        this.displayData = [];
            let itemPos: any;
    ...
    update() {
        ...
        // process frame-by-frame rendering for virtual list
    ...
    // 非虚拟路径已移除 — 保留虚拟实现 _createOrUpdateItem
    ...
    // 仅虚拟列表用（stub）
    // 为了不改变现有行为，这里保留为显式的 no-op。若将来需要启用尺寸自适应，可恢复实现。
    ...
    scrollTo(listId: number, timeInSecond: number = .5, offset: number = null, overStress: boolean = false) {
        ...
        if (t._layout && t._layout.enabled)
            t._layout.updateLayout();

    let pos: any = t.getItemPos(listId);
        if (!pos) {
        ...
        // t._scrollView.stopAutoScroll();
        if (runScroll) {
            t._scrollView.scrollToOffset(pos as any, timeInSecond);
    ...
    _calcNearestItem() {
        ...
        // virtual-only component: always calc view pos
        t._calcViewPos();
    ...
    data = t.displayData[t.displayItemNum - 1];
