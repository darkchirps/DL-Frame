import { svnAdd, svnCleanup, svnCommit, svnLog, svnRevert, svnUpdate } from "./svn";
...
class CustomMenu extends Editor.__Menu__ {
    ...
    constructor(template, webContent) {
        ...
        if (template.length > 0) {
            ...
            if (first.label == "新建") {
                //TODO 在这里插入asset右键菜单
                let assetInfo = getSelectedFirstAssetInfo() as AssetInfo;
                        ...
                        submenu: [
                            {
                                label: "更新（当前选中目录）",
                                click() {
                                    svnUpdate(assetInfo.path);
                                },
                            },
                            {
                                label: "更新（项目根目录）",
                                click() {
                                    svnUpdate(Editor.Project.path);
                                },
                            },
                        ],
                    },
                        ...
                        submenu: [
                            {
                                label: "提交（当前选中目录）",
                                click() {
                                    svnCommit(assetInfo.path);
                                },
                            },
                            {
                                label: "提交（项目根目录）",
                                click() {
                                    svnCommit(Editor.Project.path);
                                },
                            },
                        ],
                    },
                        ...
                        submenu: [
                            {
                                label: "日志",
                                click() {
                                    svnLog(assetInfo.path);
                                },
                            },
                            {
                                label: "还原",
                                click() {
                                    svnRevert(assetInfo.path);
                                },
                            },
                            {
                                label: "清理",
                                click() {
                                    svnCleanup(assetInfo.path);
                                },
                            },
                        ],
                    },
                        ...
                        submenu: [
                            {
                                label: "增加（当前选中目录）",
                                click() {
                                    svnAdd(assetInfo.path);
                                },
                            },
                            {
                                label: "增加（项目根目录）",
                                click() {
                                    svnAdd(Editor.Project.path);
                                },
                            },
                        ],
                    }
