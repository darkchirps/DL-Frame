...
/*******************************************************************************
 * 描述:    框架全局方法管理器 数组/Map相关
*******************************************************************************/
export default class GeneralArrayMap {
    ...
    */
    ...
    /**遍历坐标数组 取出相同的*/
    public findDuplicateCoordinates(coords: Array<Vec3>) {
    ...
    */
    public findMapGroup<T>(map: Map<string, T>, key: string): T[] {
        ...
        for (const [, val] of map) {
            const sign = val[key];
            if (indexMap.has(sign)) {
                return [indexMap.get(sign), val];
            } else {
                indexMap.set(sign, val);
            }
        }
        return [];
    }
